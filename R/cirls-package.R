#' @description
#' The package `cirls` provides routines to fit **generalised linear models (GLM) with linear constraints on the coefficients**. The package's framework allows to easily perform a range of common applications such as shape-constrained regression or compositional regression, but also allows specifying *any* custom linear constraint for more niche applications. The package provides dedicated methods to perform inference and model selection.
#'
#' @section Methodological framework:
#' Applying linear constraints to GLMs allow to restrict the fitted coefficients \eqn{\beta} to respect specific linear combinations. These constraints can represent assumptions on the coefficients or allow identifiability. More formally, the fitted GLM is subject to:
#' \deqn{ \mathbf{l} \leq \mathbf{C\beta} \leq \mathbf{u} }
#' where \eqn{\mathbf{C}} is a matrix encoding the constraints with each row representing a single constraint, and each column representing a variable in the model. \eqn{\mathbf{l}} and \eqn{\mathbf{u}} are lower and upper bounds for the constraints, respectively.
#'
...
#'
#' Linearly constrained coefficients follow a **truncated multivariate Normal** (TMVN) distribution, *i.e.* a multivariate normal that respects the constraints specified by \eqn{\mathbf{C}}, \eqn{\mathbf{l}} and \eqn{\mathbf{u}}. Inference is conducted by [simulating][simulCoef()] from the TMVN distribution of fitted coefficients, which can then be used to compute useful summaries like the [variance-covariance matrix][vcov.cirls()] and [confidence intervals][confint.cirls()].
#'
#' @section Usage:
#'
#' ## Fitting the model
#' The `cirls` package has been developed as a **plug-in** to [glm][stats::glm()]. The main function is [cirls.fit][cirls.fit()], designed to be used within [glm][stats::glm()] through the argument `method = "cirls.fit"`. This will fit the GLM using the CIRLS algorithm instead of the default IRLS algorithm. All arguments of [glm][stats::glm()] such as `formula`, `data` and `family` retain the same usage. Additional arguments specific to `cirls` are passed through the ellipsis `...` or the `control` argument (*but not both at the same time*). If any parameter is passed through `control`, then `...` will be ignored. See [cirls.control][cirls.control()] for the full list of arguments and examples below for how to use the package.
#'
#' ## Specifying constraints
#' Constraints can be specified through arguments `constr` and/or `Cmat`/`lb`/`ub`. The [constr][buildCmat()] argument provides a user-friendly way to pass common built-in constraints through a formula and is the recommended way for new users. See the list of available built-in constraints below. Alternatively, the arguments [Cmat/lb/ub][buildCmat()] allow more flexibility in passing constraints. These arguments take either a full constraint matrix (`Cmat`) and bound vectors (`lb` and `ub`), or named lists to specify a constrained matrix and bound vectors for specific terms in the model. See the help for [buildCmat][buildCmat()] for greater details.
#'
#' @section Functions included in the package:
#' [cirls.fit][cirls.fit()] is the workhorse performing the fitting, and is controlled by [cirls.control][cirls.control()]. The fit returns an object of class `cirls` which inherits from class `glm`. This means that all methods available for usual `glm` objects can also be used on a `cirls` object. This typically includes [coef][stats::coef()] and [summary][stats::summary.glm()] for instance. However, some methods are supplanted by new methods that are specific to `cirls` objects. See the index below for a comprehensive list of supplanted methods.
#'
#' ## Inference
...
#'
#' ## Other functions
#' [uncons][uncons()] refit the model removing some or all of the constraints, acting similarly to the [update][stats::update()] function but focusing on constraints.
#'
#' [buildCmat][buildCmat()] and [checkCmat][checkCmat()] are convenience functions that are mainly used internally in [cirls.fit][cirls.fit()]. They are not meant to be called directly by the user, but can be useful to check a manually provided full constraint matrix. Specifically, [buildCmat][buildCmat()] builds a full constraint matrix from the `constr` and/or `Cmat`/`lb`/`ub` arguments and a model frame. [checkCmat][checkCmat()] checks whether a constraint matrix can be reduced by detecting redundant and underlying equality constraints. The help of both functions also provide technical details on constraints and constraint matrices.
#'
#' @section Index:
#'
